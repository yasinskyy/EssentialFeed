...
class FeedSnapshotTests: XCTestCase {
    ...
    func test_feedWithErrorMessage() {
        ...
        assert(snapshot: sut.snapshot(for: .iPhone16(style: .light)), named: "FEED_WITH_ERROR_MESSAGE_light")
        assert(snapshot: sut.snapshot(for: .iPhone16(style: .dark)), named: "FEED_WITH_ERROR_MESSAGE_dark")
    }
    ...
    private func assert(snapshot: UIImage, named name: String, file: StaticString = #file, line: UInt = #line) {
        ...
        if snapshotData != storedSnapShotData {
            let temporarySnapstotURL = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true).appendingPathComponent(snapshotURL.lastPathComponent)
            
            try? snapshotData?.write(to: temporarySnapstotURL)
            
            XCTFail("New snapshot does not match stored snapshot. New snapshot URL: \(temporarySnapstotURL), Stored snapshot URL: \(snapshotURL)", file: file, line: line)
        }
    }
